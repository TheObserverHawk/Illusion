...
2) Il consulte le $\color{green}\text{Translation Lookaside Buffer}$ (TLB) :
	...
	3) Si l’entrée est invalide, une exception **Page Fault** est déclenchée.
		1) Le gestionnaire associé charge la page virtuelle depuis la mémoire secondaire (p.ex. disque dur).
		2) La page des tables est mise à jour.
	4) Si l’entrée est valide, Il complète le $\color{lightblue}\text{PPN}$ avec l’offset pour obtenir l’$\color{lightblue}\text{adresse physique}$
4) La lecture ou l’écriture à l’adresse physique peut alors avoir lieu
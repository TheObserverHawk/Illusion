...
On suppose que 
1. $$f \text{ est dérivable en }a$$ et $$a \in \operatorname{adh}(\operatorname{Dom}(g \circ f) \text{ \\ } \{a \}$$ où $$\operatorname{Dom}(g \circ f) = \{x \in \mathbb{R}\ |\ x \in \operatorname{Dom}f \cap x \in \operatorname{Dom}g \}$$
$\Rightarrow a \in \operatorname{adh(Dom}f) \setminus \{a\}$ 
...
On a $g(y) = \sqrt{y}$ et $f(x) = 1+ e^{x^2} \Rightarrow g(f(x)) = \sqrt{1+ e^{x^2}}$ 
...
1. Si $f$ est dérivable en $a$ et 
2. $g$ est dérivable en $f(a)$, 

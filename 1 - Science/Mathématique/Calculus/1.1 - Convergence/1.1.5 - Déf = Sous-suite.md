...
2. Construisons une fonction $\phi$ qui explique comment pioche (Caractéristique de la fonction $\phi$)
	1. $\phi$ doit être croissante

Donc $\phi$ doit être une fonction croissante i.e. $$\forall n \in \mathbb{N},\ \phi(n)\ <\ \phi(n+1)$$ car le 1.(a), 1.(b) et le 2.(a)
...
Donc on a $$\begin{align} (Y_n) \quad \quad \quad \quad (X_n) \\ \text{Indices} \quad  \varphi  \quad \text{Indices} \\ 0 \quad \longrightarrow 2 = \varphi(0) \\ 1 \quad \longrightarrow 5 = \varphi(1) \\ 3 \quad \longrightarrow 10 = \varphi(2)   \end{align}$$
...
La proposition dit qu’on a l’équivalence suivante : $$\forall n_1,\ n_2\in \mathbb{N},\ n_1 < n_2 \Rightarrow \varphi(n_1) < \varphi(n_2) \equiv \forall n \in \mathbb{N},\ \phi(n)\ <\ \phi(n+1)$$ 

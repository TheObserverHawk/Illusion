...
1. **_Cas 1 : si $a<0$_** : 
En utilisant le théorème de localité, on veut $$\underset{x\to a}{\lim}f(x)=\underset{\overset{x\to a}{x\in\ [a-r;a+r]}}{\lim}f(x)=\underset{\overset{x\to a}{x\in\ ]-\infty;0]}}{\lim}f(x)$$ Prenons $$r=\frac{|a-0|}{2}=\frac{|a|}{2}>0$$ Montrons que $$[a-r;a+r]\subseteq\ ]-\infty;0]$$ Soit $x\in\ [a-r;a+r]$, on a $$\begin{aligned}x\le a+r&=a+\frac{|a|}{2}\\&=a-\frac{a}{2}\quad\text{ , }|a|=-a\text{ car }a<0\\&=\frac{}\end{aligned}$$
# Version formelle

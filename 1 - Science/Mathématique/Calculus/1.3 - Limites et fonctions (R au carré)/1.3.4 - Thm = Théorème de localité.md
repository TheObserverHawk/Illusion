...
Si $\underset{\overset{x\to a}{x\in C_2}}{\lim}f(x)$ existe, alors $lim_\underset{\overset{x \longrightarrow a}{\operatorname*{x \in C_1 }}} {} {f(x)}$ existe
...
Montrons que $$[a-r;a+r]\subseteq\ ]0;3]$$ Soit $x\in\ [a-r;a+r]$, on a $x\ge a-r$ 
# Version formelle

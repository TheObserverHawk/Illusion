...
Prenons $$f(x)=\begin{cases}x -1  \quad \text{ si } x\ge2 \\  x^2 \quad \quad \ \ \ \text{si } x<2 \end{cases}$$ On a $\underset{\overset{x\to a}{x < 2}}{\lim}f(x)=\underset{\overset{x\to a}{x\in\ ]-\infty;2]}}{\lim}x^2=4$ où $C_1=\ ]-\infty;2]$ 

...
En utilisant le corollaire du théorème de l'exhaustivité, on a 
	- $\underset{\overset{x\to a}{<}}{\lim}f(x)\overbrace=^{\text{ car }x< 0}\underset{\overset{x\to 0}{<}}{\lim}e^x+x\overbrace=^{\text{rés. du cours + RDC}}e^0+0=1$
	- $$\underset{\overset{x\to a}{>}}{\lim}f(x)\underbrace=_{\text{car Propriété Localité}}\underset{\overset{x\to a}{\overset{>0}{x\in\ [0-1;0+1]}}}{\lim}f(x)\underbrace=_{\text{car }x\in 1\text{ et }x>0\quad\Rightarrow\quad}$$
# Version formelle
